...
function canonizePage(page, type) {
  if(!page) return page;
  console.log(type.match(/\b(x|xht|ht)ml\b/) ? "Yes" : "NO");
...
// console.log(raw_message,"===",results);
  ...
  //   regex: regex
  // }));
  // console.log(text,"=-===",regex,"callback:",callback.toString());
...
function cleanAndHashPage(html, mode, regex, selector, callback) {
  ...
  if(mode == 'regex' && regex) {
    ...
    // cleanHtmlPageByRegex(html, callBackWithCrc);
  }
}
...
//提取网页中链接
function getLinksInHtml(html,url) {
  if(html != null && html != ""){
    var link_html = $(html).find("rss").html();
      ...
      for(var i = 0;i<match.length;i++){
        ...
        if(link_rec.toLowerCase().substring(0,7)!="http://"
          ...
          if(link_rec.substring(0,1)=="/"){
            links_arr.push(host + link_rec);
          }else{
            links_arr.push(url.replace(/\/[^\/]*?$/g,"") + "/" + link_rec);
          }
        } else {
          links_arr.push(link_rec)
        }
        
        // console.log("普通链接:",link_rec);
        // console.log("-============-");
        // dao.addLink(RegExp.$1);
        // links_arr.push(link_rec);
      }
      // console.log("bbbbbbbbbbbbbbbbbbbbb",links_arr)
      dao.addLink(links_arr);
    }
  }
}
...
(function() {
  ...
  // than sorry.
  watchdog = function() {
    console.log("into watchdog:  ",Date.now()-projected_check_time,";;;WATCHDOG_TOLERANCE:  ",WATCHDOG_TOLERANCE);
