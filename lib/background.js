...
function findAndFormatSelectorMatches(html, selector, callback) {
  try {
    ...
    var result = $(selector, $(html)).map(function() {
      return $(this).html();
    }).join('\n');
...
function checkPage(url, callback, force_snapshot) {
  dao.getPage({url:url}, function(page) {
    ...
    $.ajax({
      ...
      error: function(e) {
        console.log("扫描原始链接“",url,"”出现异常，错误代码：",e.status||999,",错误信息：",e.statusText);
...
function getLinksInHtml(html,url,callback) {
  ...
  if(html != null && html != ""){
    var link_html;
    link_html = $(html).find("item").html(); //加span
    if(link_html != null && link_html != ""){
      ...
      // var match = /<a[^>]*href="([^"]*)"[^>]*>(.*?)<\/a>/g;
        var match = $(html).find("a"); //加span
      // var regex = new RegExp(match);
      for(var i = 0;i<match.length;i++){
        var link_rec = "";
        if($(match[i]).attr("href")!=undefined && $(match[i]).attr("href")!=null) {
          link_rec = $(match[i]).attr("href").toString();
        }else{
          continue;
        }
        //无脑的url拼凑过程
        if(link_rec.toLowerCase().substring(0,7)!="http://"
          ...
          if(link_rec.substring(0,1)=="/"){
            ...
            links_arr.push({"link" : url.replace(/\/[^\/]*?$/g,"")=="http:/"?host:url.replace(/\/[^\/]*?$/g,"") + "/" + link_rec});
          }
        } else {
      ...
      var times = $.map(pages, function(page) {
        if (!page.last_check) {
          ...
          var check_interval = page.check_interval ||
                               WATCHDOG_INTERVAL;
          console.log(page.url,page.last_check + check_interval - current_time);
...
module.exports = {
  ...
  "scheduleCheck": scheduleCheck,
  "getStrippedBody": getStrippedBody
};
