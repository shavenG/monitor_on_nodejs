module.exports = {
    ...
    getLinksWithPage: function(page, callback) {
        var query = "select * from links where status >=200 and status <=300 order by time desc limit "+(page-1)*PAGE_PER_COUNT+","+PAGE_PER_COUNT;
        db.all(query,function(err, rows) {
            callback(rows);
        });
    },
    //带时间的，输出使用
    getLinksWithDate: function(date, callback) {
        date = date.toString();
        var year = date.substring(0,4);
        var month = date.substring(4,6);
        var day = date.substring(6,8);
        var start_time = new Date(year, month, day).getTime();
        var end_time = start_time + 86400000;
        var query = "select * from links where status >=200 and status <=300 and time>=" + start_time + " and time<" + end_time;
        db.all(query,function(err, rows) {
            callback(rows);
        });
    },
