module.exports = {
    ...
    getCurrentDayCount: function(callback) {
        ...
        var query = "SELECT count(*) AS today_count FROM links WHERE time>"+currentDayTime+" AND time<"+nextDayTime;
        db.get(query,function(err, row) {
            callback(row);
        });
    },
    ...
    getLinksWithPage: function(page, callback) {
        var query = "select link,title,author,author_email,image,content from links order by time desc limit "+(page-1)*PAGE_PER_COUNT+","+PAGE_PER_COUNT;
        db.all(query,function(err, rows) {
            callback(rows);
        });
    },
    cleanPeddingStatus: function(callback) {
        db.run("update links set status='' where status='PEDDING'",callback);
    }
};