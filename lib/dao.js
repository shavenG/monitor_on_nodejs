module.exports = {
    ...
    getLinks: function(callback) {
        db.all("SELECT link FROM links WHERE status IS NULL", function(err, rows) {
            console.log("Reading link of Table links:",rows);
            var links = [];
            for (var i = 0; i < rows.length; i++) {
    ...
    //设置pages表中的updated字段
    setPageFlag: function(url,flag,callback) {
        var query = "UPDATE pages SET updated = ? WHERE url=?";
        db.run(query,[flag,url],(callback||$.noop)());
    }
