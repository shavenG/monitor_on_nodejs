...
function scanUrl(url,par) {
  $.ajax({
    ...
    error: function(e) {
      getLinkToDb();
      dao.updateLinkHtml(url, e.responseText, e.status||999, {}, false);
...
function getDetailInfo(html,url,par,status) {
  dao.getPage({url:par},function(page){
    try{
      var doc =$.load(BG.getStrippedBody(html),{
          xmlMode: false
      });
    }
    catch(e)
    {
      console.log(url, '解析出错');
      dao.updateLinkHtml(url, html, 999, {}, true, function(){});
      return;
	}
    //页面配置信息的默认值
    var result_obj = {
    ...
    dao.updateLinkHtml(url, html, status, result_obj, true, function(){});
  });
}
