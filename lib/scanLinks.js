...
(function(){function getDetailInfoController(html, type, pattern, what) {
  console.log("执行getDetailInfoController方法获取细节");
  var findAndFormat = (type == 'regex') ? BG.findAndFormatRegexMatches :
                                          BG.findAndFormatSelectorMatches;
  findAndFormat(html, pattern, function(results) {
    $('textarea', form).val(results);
    shadeBackground(true);
    form.fadeIn();

    BG.cleanAndHashPage(html, mode, mode_string, mode_string, function(crc) {
