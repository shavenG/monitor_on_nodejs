...
function importPagesList(bookmarks) {
  ...
  while (match = page_regex.exec(bookmarks, page_regex.lastIndex)) {
    ...
    var name = link.text() || chrome_i18n_getMessage('untitled', url);
...
function initializeIntervalSliders() {
  ...
  textbox.val(interval_min).change(function() {
    ...
    var message;
    if (val_min == 1) {
      message = chrome_i18n_getMessage('minute');
    } else {
      message = chrome_i18n_getMessage('minutes', '2');
    }
...
function initializeNotificationsTimeout() {
  ...
  $('#notifications_timeout input').val(timeout).change(function() {
    ...
    var label;
    if (val_ms > 60000) {
      label = chrome_i18n_getMessage('until_closed');
    } else {
...
function initializeSoundCreator() {
  ...
  $('#new_sound_create').click(function() {
    ...
    if (!(url && name)) {
      alert(chrome_i18n_getMessage('new_sound_prompt'));
      return;
    }
    ...
    audio_file.addEventListener('error', function() {
      alert(chrome_i18n_getMessage('new_sound_failed'));
      restoreCreateButton();
    });
...
function initializeImporter() {
  ...
  $('#import_perform', form).click(function() {
    ...
    try {
      ...
      alert(chrome_i18n_getMessage('import_error'));
      form.fadeOut();
      shadeBackground(false);
      return;
    }
    if (count) {
      var singular = chrome_i18n_getMessage('import_success_single');
      var plural = chrome_i18n_getMessage('import_success_multi',
                                          count.toString());
      ...
      alert(chrome_i18n_getMessage('import_empty'));
    }
    form.fadeOut();
    shadeBackground(false);
  });
...
function initializeGlobalChecker() {
  $('#check_all').click(function() {
    getAllPageURLs(function(pages) {
      var progress_message = chrome_i18n_getMessage('check_in_progress') + '..';
  ...
  $('.rename').live('click', function() {
    ...
    if (!page_link.is(':hidden')) {
      ...
      var cancel_button = $('<input type="button" value="' +
                            chrome_i18n_getMessage('cancel') + '" />');
      var ok_button = $('<input type="button" value="' +
                        chrome_i18n_getMessage('ok') + '" />');
...
function addPageToTable(page) {
  ...
  // Page info.
  var name = page.name || chrome_i18n_getMessage('untitled', page.url);
  if (name.length > 60) {
...
function init() {
  ...
  applyLocalization();
  $('title').text(chrome_i18n_getMessage('options_title'));
  $('.mode_test').val(chrome_i18n_getMessage('test_button'));
  $('.mode_pick').val(chrome_i18n_getMessage('pick_button'));
